...
const $dropdown = document.querySelector('#pokemon-type');
...
function handleSort(event) {
  ...
  if (selectedType === '') {
    ...
    sortPokemonByType(selectedType);
  }
}

function sortPokemonByType(type) {
  const xhr6 = new XMLHttpRequest();
  xhr6.open('GET', `https://pokeapi.co/api/v2/type/${type}`);
  xhr6.responseType = 'json';
  xhr6.addEventListener('load', function () {
    const filteredPokemon = xhr6.response.pokemon.map((poke) => poke.pokemon);

    const $rowPokemon = document.querySelector('.row-pokemon');
    $rowPokemon.innerHTML = '';

    filteredPokemon.forEach((pokemonInfo) => {
      const xhr7 = new XMLHttpRequest();
      xhr7.open('GET', pokemonInfo.url);
      xhr7.responseType = 'json';
      xhr7.addEventListener('load', function () {
        $rowPokemon.appendChild(renderPokemon(xhr7.response));
      });
      xhr7.send();
    });
  });
...
//     // Ensure modal content is removed
//     const $rowModal = document.querySelector('.row-modal');
//     if ($rowModal) {
//       $rowModal.remove();
//     }
